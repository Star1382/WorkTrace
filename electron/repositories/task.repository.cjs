...
const TASK_COLUMNS =
  'id, title, description, quadrant, status, due_date, remind_at, created_at, updated_at, completed_at';
...
function createTaskRepository(db) {
  return {
    ...
    getByDate(date) {
      return db
        .prepare(`SELECT ${TASK_COLUMNS} FROM tasks WHERE due_date = ? ORDER BY created_at ASC`)
        .all(date);
    },
    ...
    countByRange(start, end) {
      const total = db
        .prepare('SELECT COUNT(*) as count FROM tasks WHERE due_date BETWEEN ? AND ?')
        .get(start, end);
      const done = db
        .prepare(
          'SELECT COUNT(*) as count FROM tasks WHERE due_date BETWEEN ? AND ? AND status = ?'
        )
