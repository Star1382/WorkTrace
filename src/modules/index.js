...
const rawModules = Object.values(moduleFiles).flatMap((moduleExports) =>
  Object.values(moduleExports).filter(isFeatureModule)
);
...
export const featureModules = rawModules.sort((a, b) => (a.order || 0) - (b.order || 0));

export const defaultModuleKey = featureModules[0]?.key || 'today';

export const sidebarWidgets = featureModules
  .flatMap((module) =>
    (module.sidebarWidgets || []).map((widget) => ({ ...widget, moduleKey: module.key }))
  )
