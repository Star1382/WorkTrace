...
function ReportPanel({ type, date, refreshKey }) {
  ...
  const handleExport = async () => {
    ...
    const saveResult = await reportService.saveToFile(textResult.data.text, textResult.data.filename);
    ...
    <div className="max-w-4xl space-y-5">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-medium text-gray-800">{done}/{total}</span>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <div className="space-y-3">
            {report.weekly_trend.map((item) => (
              <div key={item.week} className="grid grid-cols-[88px_1fr_52px] items-center gap-3 text-sm">
                <span className="text-gray-600">{item.week}</span>
                <ProgressBar done={item.done} total={item.total} />
                <span className="text-right text-gray-700">{item.done}/{item.total}</span>
              </div>
            ))}
          </div>
        </div>
