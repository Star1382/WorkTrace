...
const TaskItem = React.memo(
  React.forwardRef(function TaskItem(
    {
      task,
      selectedTaskId,
      highlightedTaskId,
    ...
    return (
      ...
      >
        ...
        <div
          className={`w-1 h-12 rounded-full ${QUADRANT_COLORS[task.quadrant] || 'bg-gray-300'}`}
        ></div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2">
            <span
              className={`text-xs px-2 py-0.5 rounded ${QUADRANT_BADGE[task.quadrant] || QUADRANT_BADGE[0]}`}
            >
              {QUADRANT_LABELS[task.quadrant] || QUADRANT_LABELS[0]}
            </span>
            <span
              className={`text-xs px-2 py-0.5 rounded ${STATUS_BADGE[task.status] || 'bg-gray-100 text-gray-600'}`}
            >
